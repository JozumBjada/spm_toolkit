...
class Cut1D:
    ...
    def create_cutzxy(self, spmdata, posz, posx, posy,
        delta=1, deltaz=1, interpolate=0):
        """create 1D cut through spmdata
        
        MAIN DIFFERENCE BETWEEN create_cutzxy AND create_cut is
        in arguments both routines take. THE FORMER TAKES ALREADY
        THREE LISTS OF COORDINATES, THE LATTER IS MORE VERSATILE
        ...
        interpolate - the order of interpolation ranging
            from 0 to 5
        delta - sampling step in x- and y-direction;
            ...
            step, this step might differ in the end
        deltaz - sampling step in z-direction, for more see 'delta'
        """
        
        # sampling points creation
        self.posx, self.posy, self.posz = self._process_cut_coords(
            posz, posx, posy, delta, deltaz)

        # creation of the cut itself
        self.create_cutzxy_raw(spmdata,
            self.posz, self.posx, self.posy,
            interpolate=interpolate)    
        
    def create_zcut(self, spmdata, positions, deltaz=1,
        interpolate=0):
        """create 1D cut through spmdata is z-direction
        
        spmdata - SPMdata instance through which the cut is done
        positions - unlike in create_cut, in this function
            'positions' is a sequence of 2 numbers [x, y]
            specifying position of the z-cut
        delta - sampling step in x- and y-direction;
            e.g. 'delta' = 3 means that approx. every third data
            point will be taken, value of 'delta' is only an
            advice for the routine to calculate actual sampling
        ...
        deltaz - sampling step in z-direction, for more see 'delta'
        interpolate - the order of interpolation ranging
            from 0 to 5; for 'interpolate' > 1 there might be
            problems with interpolation and NaNs are returned
            instead of numbers
        """
        ...
        self.create_cut(spmdata, [positions], append=True,
            deltaz=deltaz, interpolate=interpolate)
        
    def create_cut(self, spmdata, positions, delta=1, deltaz=1,
        append=True, interpolate=0):
        """create 1D cut through spmdata
        ...
        positions - sequence of 2-tuples or 3-tuples of point
            coordinates, these coordinates express indices of
            respective points in the spmdata array, but need not
            be integers, in which case interpolation is used,
            see 'interpolate', BEWARE: these are NOT sampling
            point, but only control points between which the
            points should be sampled, see 'delta' and 'deltaz'
            2-tuples case: (x, y) - x- and y-coordinate, only
                first len(spmdata) elements of 'positions' is
                used, the first 2-tuple is assumed to lie in
                the first layer, the second 2-tuple is assumed
                to lie in the second layer, etc., in this case,
                sampling of z-coordinates is not performed
            3-tuples case: (z, x, y) - the first coordinate is
                the z-coordinate, no truncation is performed
        delta - sampling step in x- and y-direction;
            e.g. 'delta' = 3 means that approx. every third data
            point will be taken, value of 'delta' is only an
            advice for the routine to calculate actual sampling
            step, which might differ in the end
        deltaz - sampling step in z-direction, for more see 'delta'
        append - option valid only for 2-tuple positions;
            if True, then the last position point is repeated so as
            the lenght of the cut is equal to the number of layers
            present in spmdata array
        interpolate - the order of interpolation ranging
            from 0 to 5;
            0 case: effectively no interpolation, take the
                nearest point
        ...
        self.posx, self.posy, self.posz = self._process_cut_coords(
            poszl, posxl, posyl, delta, deltaz)
    ...
    def _process_cut_coords(self, posz, posx, posy,
        delta, deltaz):
        """based on control points stored in posz, posx, posy
        create sampling points and store them
        in posxl, posyl, poszl
        ...
        if minlen != max(len(posx), len(posy), len(posz)):
            print("_process_cut_coords: Unequal lengths of"
                " coordinate lists. Only first "
                "{} elements used.".format(minlen))
    ...
    def create_cutzxy_raw(self, spmdata, posz, posx, posy,
        interpolate=0):
        """create 1D cut through spmdata
        ...
        minlen = min(len(posx), len(posy), len(posz))
        if minlen != max(len(posx), len(posy), len(posz)):
            print("create_cutzxy_raw: Unequal lengths of"
                " coordinate lists. Only first"
                " {} elements used.".format(minlen))
...
#            return
        ...
        for chan, val in spmdata.channels.items():
            ...
            # x-ove a y-ove souradnice
            self.channels[chan] = ndimage.map_coordinates(val,
                [self.posz, self.posy, self.posx], 
...
#            self.channels[chan] = val[self.posz, self.posy, self.posx]   
    ...
    def show_channels(self, *cl, tdata=None, linespec="b-",
        rescale=False):
        ...
        for i, chan in enumerate(chanlist):
            if tdatal is None:
                ...
                ydatal = [y for x, y in
                    sorted(zip(tdatal, self.channels[chan]))]
